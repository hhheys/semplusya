...
def get_timetable(day, class_number): # day : 0 - Mon, 1 - Tue, 2 - Wed, 3 - Thu, 4 - Fri, 5 - Mon (next week)
    ...
    print('https://sheets.googleapis.com/v4/spreadsheets/{}?includeGridData=true&ranges={}&key={}'.format(config.GOOGLE_SHEET,column + str(row) + ":" + column + str(row + 8),config.API_KEY))
    res = requests.get('https://sheets.googleapis.com/v4/spreadsheets/{}?includeGridData=true&ranges={}&key={}'.format(config.GOOGLE_SHEET,column + str(row) + ":" + column + str(row + 7),config.API_KEY))
